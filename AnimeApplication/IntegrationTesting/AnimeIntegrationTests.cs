...
public class AnimeIntegrationTests(CustomWebApplicationFactory factory) : IClassFixture<CustomWebApplicationFactory> {
    ...
    public async Task GetById_ExistingId_ReturnsOkWithDto() {
        var response = await _client.GetAsync($"/api/animes/{CustomWebApplicationFactory.ExistingAnimeId1}", TestContext.Current.CancellationToken);
    ...
    public async Task Update_ExistingId_ReturnsOkWithUpdatedAnime() {
        ...
        var response = await _client.PatchAsync($"/api/animes/{CustomWebApplicationFactory.ExistingAnimeId2}", form, TestContext.Current.CancellationToken);
